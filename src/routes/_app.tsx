...
function AppLayout() {
  ...
  return (
    <div className="relative min-h-screen">
      ...
      {/* sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-40 w-72 transform transition-transform lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
        <div className="m-3 h-[calc(100vh-1.5rem)] rounded-2xl glass-strong p-5 flex flex-col">
          <div className="flex items-center gap-3 px-2">
            <div className="relative">
              <div className="absolute inset-0 rounded-xl blur-md opacity-70" style={{ background: "var(--gradient-primary)" }} />
            ...
            <div>
              <div className="text-sm font-bold tracking-wide"><span className="text-gradient">NEXUS</span> VAULT</div>
          ...
          <nav className="mt-8 flex-1 space-y-1">
            {nav.map(({ to, label, icon: Icon }) => {
              ...
              return (
                ...
                >
                  {active && <span className="absolute left-0 top-1/2 h-6 -translate-y-1/2 w-[3px] rounded-r-full" style={{ background: "var(--gradient-primary)" }} />}
          ...
          <button onClick={logout}
            className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition">
      ...
      <button onClick={() => setOpen(!open)}
        className="fixed top-4 right-4 z-50 lg:hidden glass-strong rounded-xl p-2.5">
